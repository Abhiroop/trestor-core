...
namespace TNetD.Tree
{
    ...
    class HashTree
    {
        ...
        /*
        public bool DirtyUp(TreeNode<LeafDataType> LeafNode)
        {
            TreeNode<LeafDataType> TempRoot = LeafNode;

            int HLEN = TempRoot.ID.Hex.Length << 1; // Multiply by 2            
        ...
        */
        ...
        private void TraverseLevelOrder(TreeNode<LeafDataType> Root, ref int FoundNodes)
        {
            List<TreeNode<LeafDataType>> nodes = new List<TreeNode<LeafDataType>>();
            ...
            for (int d = 0; d < 64; d++)
            {
                List<TreeNode<LeafDataType>> _nodes = new List<TreeNode<LeafDataType>>();

                foreach (TreeNode<LeafDataType> node in nodes)
                {
                    for (int i = 0; i < 16; i++)
                    {
                        if (node.Children[i] != null)
                        {
                            ...
                            for (int j = 0; j < d; j++) sb.Append("");

                            //DisplayUtils.Display(sb.ToString() +  "LO ID : " + HexUtil.ToString(node.Children[i].ID.Hex) + " : " + d);
                            // sw.WriteLine(sb.ToString() + "LO ID : " + HexUtil.ToString(node.Children[i].ID.Hex) + " : " + d);

                            _nodes.Add(node.Children[i]);
                        }
                        else
                        {
                            //DisplayUtils.Display("LO ID: NULL --------------- =============== ------------- : " + d);
                        }
                    }
                }
            ...
            //sw.Close();
            //DisplayUtils.Display(" DONE_FILE ");
        }
